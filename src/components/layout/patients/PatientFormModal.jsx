...
function ModalContent({
    ...
    const steps = useMemo(() => {
        ...
        if (isEdit) {
            base.push({ key: "historial", label: "Historial clínico" });
        }
    ...
    const initialStepIndex = useMemo(() => {
        const found = steps.findIndex((item) => item.key === initialStepKey);
    ...
    }, [initialStepIndex, patient?.id, mode]);
    ...
    const canGoNext = useMemo(() => {
        ...
        if (currentStepKey === "docs") {
            return form.docs.consentimiento && form.docs.privacidad;
        }
        ...
        <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-slate-950/55 p-2 sm:p-4">
            ...
            <div className="relative z-10 flex max-h-[96dvh] w-full max-w-[98vw] flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl sm:max-w-6xl 2xl:max-w-7xl">
                ...
                <form onSubmit={handleSubmit} className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5 md:p-6">
                    {currentStepKey === "basicos" && <StepDatosBasicos form={form} updateField={updateField} />}

                    {currentStepKey === "dolor" && <StepDolorCorporal form={form} updateField={updateField} />}

                    {currentStepKey === "antecedentes" && (
                        <StepAntecedentes
                            form={form}
                    ...
                    {currentStepKey === "docs" && <StepDocumentos form={form} updateDoc={updateDoc} />}
                    ...
                    {currentStepKey === "confirm" && <StepConfirmacion form={form} />}
            ...
            >
                <div className="space-y-3">
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                        Para continuar, deja marcados al menos <b>Consentimiento informado</b> y <b>Aviso de privacidad</b>.
                    </div>
                </div>
            </SectionBox>
        </div>
