// src/components/layout/terapia/ReservationModal.jsx
...
export function ReservationModal({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const servicesDataRaw = await servicesResp.json();
        const profsDataRaw = await profsResp.json();
  ...
  useEffect(() => {
    ...
    async function loadPayments() {
      try {
        ...
        setLastPagoId(mappedLines.length ? mappedLines[mappedLines.length - 1].id : null);
  ...
  async function syncPaymentsForCita(citaId) {
    ...
    const originalById = new Map(
      originalLines.filter((line) => line.id).map((line) => [Number(line.id), line])
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLastPagoId(paymentResult?.lastId || null);

      const refreshed = await onRefreshAppointment?.(savedCitaId);
      const paidNow =
        Boolean(refreshed?.pagado) ||
        Boolean(refreshed?.paid) ||
        remainingInternal <= 0;
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="md:col-span-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <div className="mt-2 text-[10px] text-slate-500">
                      Si el abono liquida el total, el ticket se descargará automáticamente al guardar.
                    </div>
                  </div>
